...
// Next param (after signup). Default profile.php
$next = $_GET['next'] ?? $_POST['next'] ?? 'profile.php';
$allowedNext = ['profile.php', 'dashboard.php'];
if (!in_array(basename($next), $allowedNext, true)) $next = 'profile.php';

$msg_error = '';
$msg_info  = '';

// Separate OTP context for signup
$CTX_KEY = 'signup_otp_ctx';
if (!isset($_SESSION[$CTX_KEY])) {
    $_SESSION[$CTX_KEY] = [
        'hash' => null,
        'mobile_e164' => null,
        'expires_at' => 0,
...
$ctx = &$_SESSION[$CTX_KEY];

function reset_ctx(string $key): void {
    $_SESSION[$key] = [
        'hash' => null,
        'mobile_e164' => null,
        'expires_at' => 0,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && hash_equals($csrf, $_POST['csrf'] ?? '')) {
    ...
    if ($action === 'signup') {
        // Single-field OTP or 4 boxes (compat)
        if (isset($_POST['otp'])) {
        ...
        if (!preg_match('/^\d{' . OTP_LENGTH . '}$/', $otp_input)) {
            ...
            if ($ctx['attempts'] > 5) {
                ...
                if (password_verify($otp_input, $ctx['hash'])) {
                    ...
                    try { user_create_if_not_exists($_SESSION['auth_user']); } catch (Throwable $e) {}
                    reset_ctx($CTX_KEY);
                    $dest = $_POST['next'] ?? 'profile.php';
                    $dest = in_array(basename($dest), $allowedNext, true) ? basename($dest) : 'profile.php';
                    header('Location: ' . $dest);
                    exit;
                } else {
                    $msg_error = 'Signup failed. Incorrect OTP.';
                }
            }
        }
    }
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .card { width: 100%; max-width: 460px; background: var(--card); border-radius: 14px; box-shadow: 0 10px 30px rgba(2,8,23,.08); padding: 20px; }
        ...
        .btn { display:block; width:100%; padding:14px 16px; border:0; border-radius:12px; background:var(--primary); color:#fff; font-size:16px; font-weight:600; cursor:pointer; box-shadow:0 6px 16px rgba(37,99,235,.25); }
...
<div class="wrap">
    <div class="card" role="region" aria-label="Signup">
        <div class="topbar">
            <a class="top-link" href="login.php<?php echo $next ? ('?next=' . urlencode($next)) : ''; ?>">Have an account? Login</a>
        </div>

        <h1>Create Your Account</h1>
        <div class="sub">Sign up with your phone number to continue</div>
        ...
        <?php else: ?>
            ...
            <div class="top-gap">
                ...
                <form method="post" id="otpSignupForm" autocomplete="off" novalidate class="top-gap">
                    <input type="hidden" name="csrf" value="<?php echo htmlspecialchars($csrf); ?>">
                    ...
                    <button class="btn secondary top-gap" type="submit" id="signupBtn">Sign Up</button>
                </form>
...
<script>
    ...
    // Phone digits + enable Send OTP when 10 digits
    ...
    // OTP input: auto-limit and auto-submit
    const otp = document.getElementById('otp');
    const verifyBtn = document.getElementById('signupBtn');
    if (otp && verifyBtn) {
